...
# Define the keystore path (same as in Rust)
...
def load_hips_keypair(keystore_path: str) -> Keypair:
    ...
    try:
        ...
        logger.debug(f"Loaded seed phrase from {key_path}")
...
async def call_update_pin_and_storage_requests(requests: List[Dict[str, Any]]) -> bool:
    ...
    Args:
        requests (List[Dict[str, Any]]): List of storage request updates, where each dict contains:
            - miner_pin_requests: List of MinerProfileItem dicts with miner_node_id (str), cid (str), files_count (int)
            - storage_request_owner: AccountId (str, SS58 address)
            - storage_request_file_hash: FileHash (str, e.g., IPFS CID)
            - file_size: u128 (int)
            - user_profile_cid: FileHash (str, e.g., IPFS CID)
    ...
    try:
        ...
        call = substrate.compose_call(
            call_module='IpfsPallet',  # Adjust module name based on your pallet
        ...
        logger.error(f"Unexpected error during extrinsic submission: {e}")
        return False
    finally:
...
async def call_update_unpin_and_storage_requests(requests: List[Dict[str, Any]]) -> bool:
    ...
    Args:
        requests (List[Dict[str, Any]]): List of storage unpin requests, where each dict contains:
            - miner_pin_requests: List of MinerProfileItem dicts with miner_node_id (str), cid (str), files_count (int)
            - storage_request_owner: AccountId (str, SS58 address)
            - storage_request_file_hash: FileHash (str, e.g., IPFS CID)
            - file_size: u128 (int)
...
async def call_update_pin_check_metrics(miners_metrics: List[Dict[str, Any]]) -> bool:
    """Calls the update_pin_check_metrics extrinsic on the Substrate node using the HIPS key for signing.

    Args:
        miners_metrics (List[Dict[str, Any]]): List of miner pin metrics, where each dict contains:
            - node_id: str (e.g., node ID as a string)
            - total_pin_checks: u32 (int)
            - successful_pin_checks: u32 (int)
    ...
    try:
        ...
        # Format the miners_metrics to match the MinerPinMetrics structure
        formatted_metrics = []
        ...
        # Prepare the call parameters
        call = substrate.compose_call(
            call_module='execution',  # Adjusted to execution pallet
