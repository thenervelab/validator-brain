...
async def bulk_db_insert(
    db_pool, table_name: str, records: List[Dict], 
    batch_size: int = 1000, upsert: bool = False
) -> int:
    """
        ...
        async with conn.transaction():
            for i in range(0, len(records), batch_size):
                ...
                for j in range(len(batch)):
                    placeholders.append(f"({', '.join([f'${j*len(columns) + k + 1}' for k in range(len(columns))])})")
...
async def bulk_update_miner_profiles(db_pool, miner_profiles: List[Dict]) -> int:
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            for miner_id, profiles in profiles_by_miner.items():
                ...
                await conn.execute(
                    "DELETE FROM miner_profile WHERE node_id = $1",
                    miner_id
                )
                ...
                if updated_count % 100 == 0:
                    ...
                    logger.info(
                        f"Updated {updated_count} profiles at {rate:.1f} profiles/sec"
                    )
    ...
    rate = updated_count / elapsed if elapsed > 0 else 0
    logger.info(
        f"Updated {updated_count} miner profiles in {elapsed:.1f} seconds "
