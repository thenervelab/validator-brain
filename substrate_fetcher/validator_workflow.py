...
from app.services.profile_manager import prepare_miner_profile_updates, upload_json_to_ipfs
from app.services.storage_processor import score_miners, select_miners_for_request, update_miner_scores
...
from substrate_fetcher.epoch_phases import EpochPhase, EpochPhaseManager
...
class ValidatorWorkflow:
    ...
    def determine_epoch_position(self, block_number: int, epoch_start_block: int = None) -> str:
        ...
        if position_in_epoch < 5:
            return "early"
        elif position_in_epoch < 40:
            return "mid"
        elif position_in_epoch < 98:
            return "assignment"
        else:
            return "cleanup"
    
    async def process_storage_requests(
        self, storage_requests: list, miner_profiles: list, node_registration: list = None
    ) -> tuple:
        """
    ...
    async def process_blockchain_data(self, substrate_data: Dict) -> Dict[str, List]:
        ...
        # Process node registration data
        node_registration_data = substrate_data.get("Registration.NodeRegistration", [])
        processed_data["node_registration"] = [
            NodeRegistration.from_substrate(reg_entry) 
            for reg_entry in node_registration_data
        ]
        logger.info(f"Processed {len(processed_data['node_registration'])} node registration entries")
        
        # Process miner profiles
        miner_profile_data = substrate_data.get("IpfsPallet.MinerProfile", [])
        node_metrics = substrate_data.get("ExecutionUnit.NodeMetrics", {})
        miner_files_size = substrate_data.get("IpfsPallet.MinerTotalFilesSize", {})
        miner_files_pinned = substrate_data.get("IpfsPallet.MinerTotalFilesPinned", {})
        ...
        storage_request_data = substrate_data.get("IpfsPallet.UserStorageRequests", [])
        processed_data["storage_requests"] = [
            StorageRequest.from_substrate(req) 
            for req in storage_request_data
        ]
        logger.info(f"Processed {len(processed_data['storage_requests'])} storage requests")
        
        # Process current validator
        current_validator_data = substrate_data.get("IpfsPallet.CurrentEpochValidator")
    ...
    async def perform_early_epoch_actions(self, substrate_data: Dict, block_position: int = None) -> Dict:
        """
        ...
        return {
            "profile_tables_refreshed": True,
            "block_position": block_position
        }
                
        # Verify file sizes before processing storage requests
        from substrate_fetcher.file_verification import verify_file_sizes, get_slashing_recommendations
        ...
        if slashing_recommendations:
            logger.warning(f"Found {len(slashing_recommendations)} users to slash for misrepresenting file sizes")
    ...
    async def perform_mid_epoch_actions(self, substrate_data: Dict, block_position: int = None) -> Dict:
        """
        ...
        Returns:
            Dictionary with results of actions taken
        """
        logger.info(f"Performing mid-epoch actions - IPFS data fetching (block position {block_position})")
    ...
    async def perform_assignment_epoch_actions(self, substrate_data: Dict, results: Dict, block_position: int = None) -> Dict:
        """
        ...
        # Special handling for position 90 - comprehensive miner profile reconstruction
        if block_position and block_position == 90:
            from substrate_fetcher.miner_profile_manager import reconstruct_miner_profiles, prepare_comprehensive_miner_updates
            
            logger.info(f"Block {block_number} detected: Performing comprehensive miner profile reconstruction")
            ...
            offline_miners = [
                profile for profile in reconstructed_profiles
            ...
            if offline_miners:
                ...
                updated_user_profiles, updated_miner_profiles = await update_profiles_after_redistribution(
                ...
                # Update miner updates with redistributed profiles
                miner_updates = await prepare_comprehensive_miner_updates(updated_miner_profiles_list)
        ...
        offline_miners = [
            profile
            for profile in substrate_data["miner_profiles"]
            if not profile["is_online"]
        ]
    ...
    async def perform_cleanup_epoch_actions(self, substrate_data: Dict, results: Dict, block_position: int = None) -> Dict:
        """
    ...
    async def perform_validator_actions(
        ...
        from substrate_fetcher.monitoring import create_performance_tracker, record_validation_metrics
        ...
        try:
            ...
            if epoch_position == "early":
                ...
                logger.info(f"Executing assignment epoch actions (blocks 40-98, position {block_position})")
            ...
            if epoch_position == "assignment" and "pin_requests" in results:
                logger.info(f"Submitting to blockchain during assignment phase (block position {block_position})")
                
                # Import blockchain submission
                from substrate_fetcher.blockchain_submission import submit_pending_data_to_blockchain
            ...
            if 'performance_tracker' in locals():
                metrics = performance_tracker.complete(
                    success=False, 
                    error_message=str(e)
                )
                await record_validation_metrics(metrics)
        ...
        async with db_pool.acquire() as conn:
            ...
            miner_profiles = await conn.fetch(
                ...
                SELECT DISTINCT 
                    r.node_id, 
                    r.ipfs_peer_id,
                    COALESCE(ms.is_online, false) as is_online,
            ...
            storage_requests = await conn.fetch(
                """
            ...
            if processed_data["miner_profiles"]:
                ...
                async with db_pool.acquire() as conn:
                    async with conn.transaction():
                        ...
                        # Store miner profiles (only for registered nodes)
                        for miner in processed_data["miner_profiles"]:
                            if hasattr(miner, 'profile_cid') and miner.profile_cid:
                                ...
                                if node_exists:
                                    ...
                                    logger.warning(f"Skipping miner profile for unregistered node: {miner.node_id}")
                ...
                if isinstance(current_validator_data, dict) and "account_id" in current_validator_data:
                    ...
                    logger.warning(f"Unexpected CurrentEpochValidator format: {current_validator_data}")
    
                # MOCKED: Always act as the current validator for testing
                logger.info(f"MOCK: Acting as current validator at block #{current_block.number} (actual validator: {current_validator})")
