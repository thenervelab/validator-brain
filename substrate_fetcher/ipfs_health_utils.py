...
async def perform_ipfs_ping(
    ...
    stop_event=None,
    availability_manager: FileAvailabilityManager = None,
):
    """
    ...
    )  # Fallback to a new event if none provided
    ...
    params = {
        "arg": ipfs_peer_id,
        "count": "1",
    }  # count must be a string for query params
    ...
    try:
                ...
                if response.status == 200:
                    async for line in response.content:
                        if stop_event.is_set():  # Check for shutdown signal
                            logger.warning(
                                f"Shutdown signal received during ping for {node_id}"
                            )
                            return  # Exit early if shutdown is signaled
                        try:
                            data = json.loads(line.decode("utf-8"))
                            if data.get("Success") and (
                                data.get("Time") or data.get("AvgLatency")
                            ):
                                ping_successful = True
                                break  # Found success signal
                        except json.JSONDecodeError:
                            logger.debug(
                                f"Non-JSON line from IPFS ping for {node_id}: {line}"
                            )
        ...
        logger.error(
            f"IPFS connection error for {ipfs_peer_id} (Node: {node_id}): {e_conn}"
        )
    ...
    async with db_pool.acquire() as conn:
        try:
            ...
            if ping_successful:
                logger.info(
                    f"Successfully pinged and updated/inserted health for {node_id} (IPFS: {ipfs_peer_id}) in epoch {epoch_number}."
                )
            else:
                logger.warning(
                    f"Failed to ping {node_id} (IPFS: {ipfs_peer_id}) in epoch {epoch_number}. Updated/inserted health stats."
                )
        except Exception as e_db:
            logger.error(
                f"Database error updating/inserting health for {node_id} (IPFS: {ipfs_peer_id}) in epoch {epoch_number}: {e_db}"
            )


async def perform_ipfs_pin_check(
    db_pool: asyncpg.Pool,
    node_id: str,
    ipfs_peer_id: str,
    root_cid_to_check: str,
    epoch_number: int,
    stop_event=None,
    availability_manager: FileAvailabilityManager = None,
):
    ...
    if db_pool is None:
        logger.error(
            f"Database pool not initialized. Cannot perform pin check for {node_id}."
        )
        return
    ...
    logger.info(
        f"Starting pin check for {node_id} (IPFS: {ipfs_peer_id}) on root CID {root_cid_to_check} in epoch {epoch_number}..."
    )
    ...
    refs_api_url = f"{config.get_dht_node_url()}/api/v0/refs"
    ...
    try:
        ...
        logger.error(
            f"Connection error fetching refs for {root_cid_to_check} (Node: {node_id}): {e_conn}. Will check root CID directly."
        )
    except Exception as e_req:
        logger.error(
            f"Request error fetching refs for {root_cid_to_check} (Node: {node_id}): {e_req}. Will check root CID directly."
        )

    if all_block_cids:
        effective_cid_checked = random.choice(list(all_block_cids))
        logger.info(
            f"Selected random block {effective_cid_checked} (from {len(all_block_cids)} blocks) for DHT check for {node_id} (Root CID: {root_cid_to_check})."
        )
    else:
            ...
            async with session.post(dht_api_url, params=dht_params) as response:
                if response.status == 200:
                    async for line in response.content:
                        ...
                        try:
                            ...
                            if data.get("Type") == 4 and data.get("ID") == ipfs_peer_id:
                                logger.info(
                                    f"SUCCESS: Miner {node_id} (IPFS: {ipfs_peer_id}) IS a provider for {effective_cid_checked}."
                                )
                            ...
                            if data.get("Type") == 4 and data.get("Responses"):
                                for resp_peer in data["Responses"]:
                                    if resp_peer.get("ID") == ipfs_peer_id:
                                        logger.info(
                                            f"SUCCESS: Miner {node_id} (IPFS: {ipfs_peer_id}) IS a provider (found in Responses) for {effective_cid_checked}."
                                        )
                                        pin_check_successful = True
                                        break
                    ...
                    if not pin_check_successful:
                        logger.warning(
                            f"FAILURE: Miner {node_id} (IPFS: {ipfs_peer_id}) is NOT listed as a provider for {effective_cid_checked} after checking all stream results."
                        )
                else:
                    error_text = await response.text()
                    logger.warning(
...
async def run_all_health_checks_for_epoch(
    ...
    logger.info(
        f"Completed one round of orchestrated health checks for epoch {epoch_number}."
    )
