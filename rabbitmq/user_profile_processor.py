"""
RabbitMQ processor for listening to substrate chain user profiles.

This processor:
1. Connects to the substrate chain
2. Listens for ipfsPallet.userProfile storage changes
3. Parses the user profile data
...
class UserProfileProcessor:
    """Processor that listens to substrate chain for user profiles and sends them to RabbitMQ."""
    ...
    def parse_user_profile_data(self, storage_data: list[list[Any]]) -> list[dict[str, str]]:
        ...
        Returns:
            List of parsed user profile dictionaries
        """
        parsed_profiles = []
    ...
    async def fetch_and_process_profiles(self):
        ...
        try:
            ...
            for key, value in result:
                # key is the account, value is the CID
                storage_data.append([[str(key.value)], str(value.value)])

            logger.info(f"Fetched {len(storage_data)} user profiles from substrate")
...
async def cleanup_orphaned_files():
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            deleted_files_count = await conn.execute("""
                DELETE FROM files f
                WHERE NOT EXISTS (
            ...
            # Delete orphaned file assignments
            deleted_assignments_count = await conn.execute("""
                DELETE FROM file_assignments fa
                WHERE NOT EXISTS (
                    SELECT 1 FROM active_files_from_chain a
                    WHERE a.cid = fa.cid
                )
            ...
            logger.info(f"Deleted {deleted_files_count} orphaned files from 'files' table")
            logger.info(f"Deleted {deleted_assignments_count} orphaned assignments from 'file_assignments' table")
