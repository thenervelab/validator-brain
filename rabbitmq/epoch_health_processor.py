...
class EpochHealthProcessor:
    ...
    async def clear_epoch_health_data(self, current_epoch: int) -> None:
        ...
        async with self.db_pool.acquire() as conn:
            try:
                ...
                if updated_count > 0:
                    logger.info(
                        f"✅ PRESERVED health data for PREVIOUS epoch {previous_epoch} ({updated_count} records marked as stale)"
                    )
                else:
                    logger.info(
    ...
    async def process_epoch_health_checks(self) -> None:
        ...
        try:
            ...
            logger.info(
                f"Successfully queued {queued_miners} miners with {total_files} total files for comprehensive epoch health checks in epoch {current_epoch}"
            )
