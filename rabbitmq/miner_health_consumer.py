...
from app.db.queries.file_assignment_queries import (
    remove_miner_from_assignments,
    get_miner_file_count,
)
...
class MinerHealthConsumer:
    ...
    async def update_health_results(
        self,
        node_id: str,
        epoch: int,
        ping_success: bool,
        ...
        async with message.process():
            ...
            async with self.miner_semaphore:
                try:
                    ...
                    if success:
                        logger.debug(
                            f"Successfully processed health check for {message_data.get('node_id', 'unknown')} in {processing_time:.2f}s"
                        )
                    else:
                        logger.error(
                            f"Failed to process health check for {message_data.get('node_id', 'unknown')} after {processing_time:.2f}s"
                        )
