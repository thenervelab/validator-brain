...
logger = logging.getLogger("file-assignment-processor")
...
class FileAssignmentProcessor:
    def __init__(self):
        ...
        self.max_reassignments_per_batch = int(
            os.getenv("MAX_REASSIGNMENTS_PER_BATCH", "50")
        )
    ...
    async def get_files_needing_reassignment(self) -> List[Dict[str, Any]]:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            if files_with_nulls:
                ...
                completely_empty = [
                    f for f in files_with_nulls if f["null_miner_count"] == 5
                ]
                partially_empty = [
                    f for f in files_with_nulls if f["null_miner_count"] < 5
                ]
                ...
                logger.info(
                    f"🔍 Found {len(files_with_nulls)} files needing reassignment:"
                )
                logger.info(
                    f"   - {len(completely_empty)} completely empty (0/5 miners)"
                )
                logger.info(
                    f"   - {len(partially_empty)} partially empty (1-4/5 miners)"
                )

                if completely_empty:
                    logger.info(
                        f"🎯 Prioritizing {len(completely_empty)} completely empty files:"
                    )
                    for file_info in completely_empty[:3]:  # Show first 3
                        logger.info(
                            f"     {file_info['cid'][:20]}... (all NULL miners)"
                        )
    ...
    def select_miners_for_file(
        ...
        for miner in miners:
            ...
            if ipfs_repo_size > 0:
                ...
                if calculated_size > ipfs_repo_size:
                    logger.debug(
                        f"Miner {miner['node_id']}: Our calculated size ({calculated_size:,}) > IPFS repo size ({ipfs_repo_size:,}), using calculated"
                    )
                    used_storage = calculated_size
            else:
                ...
                used_storage = calculated_size
                logger.debug(
                    f"Miner {miner['node_id']}: No IPFS repo size data, using calculated size ({calculated_size:,})"
                )
            ...
            if available_storage >= required_space:
                ...
                logger.debug(
                    f"Miner {miner['node_id']}: ✅ {available_storage:,} available >= {required_space:,} required"
                )
            else:
                logger.debug(
                    f"Miner {miner['node_id']}: ❌ {available_storage:,} available < {required_space:,} required"
        ...
        if len(suitable_miners) < self.replicas_per_file:
            logger.warning(
                f"Only {len(suitable_miners)} suitable miners found for file size {file_size:,} bytes (need {required_space:,} with safety margin), "
        ...
        selected_miners = self._weighted_random_selection(
            scored_miners, self.replicas_per_file
        )
    ...
    def _weighted_random_selection(
        ...
        for _ in range(count):
            ...
            selected_idx = random.choices(
                range(len(remaining_miners)), weights=remaining_probs
            )[0]
    ...
    def select_miners_for_reassignment(
        ...
        if len(suitable_miners) < empty_slots:
            logger.warning(
                f"Only {len(suitable_miners)} suitable miners found for reassignment (need {required_space:,} bytes with safety margin), "
                f"need {empty_slots}"
            )
    ...
    def update_miner_usage(
        ...
        for miner in miners:
            if miner["node_id"] in selected_miner_ids:
                ...
                miner_id = miner["node_id"]
                self.batch_assignments[miner_id] = (
                    self.batch_assignments.get(miner_id, 0) + 1
                )

                logger.debug(
                    f"Updated miner {miner['node_id']}: +{file_size} bytes, +1 file, "
    ...
    async def process_new_file_assignments(
        ...
        for file_info in pending_files:
            try:
                ...
                selected_miners = self.select_miners_for_file(
                    available_miners, file_size
                )

                # CRITICAL VALIDATION: Check if we have enough miners
                if len(selected_miners) < min_required_miners:
                    logger.error(
                        f"❌ INSUFFICIENT MINERS for file {cid}: Found {len(selected_miners)}, need {min_required_miners}"
                    )
                ...
                if len(selected_miners) < self.replicas_per_file:
                    logger.warning(
                        f"⚠️ Only assigned {len(selected_miners)} replicas for file {cid}, "
                ...
                logger.info(
                    f"✅ Successfully queued assignment for file {cid[:16]}... to {len(selected_miners)} miners: "
                    f"{', '.join(selected_miners[:3])}{'...' if len(selected_miners) > 3 else ''}"
                )

            except Exception as e:
                logger.error(
                    f"❌ Error processing file {file_info.get('cid', 'unknown')}: {e}"
                )
                failed_assignments += 1
                continue

        logger.info(
            f"📊 Assignment Summary: {successful_assignments} successful, {failed_assignments} failed (insufficient miners)"
        )
    ...
    async def process_reassignments(
        ...
        logger.info(
            f"Found {len(files_needing_reassignment)} files needing reassignment"
        )
        ...
        for file_info in files_needing_reassignment:
            try:
                ...
                if not new_miners:
                    logger.warning(
                        f"No suitable miners found for reassignment of file {cid}"
                    )
    ...
    async def process_failing_miner_replacements(
        ...
        """
        files_with_failing_miners = await self.get_files_with_failing_miners(
            current_epoch
        )
        ...
        for file_info in files_with_failing_miners:
            try:
                ...
                healthy_miners = [
                    ...
                    if m is not None and m not in failing_miners
                ]
                ...
                if not replacement_miners:
                    logger.warning(
                        f"No suitable replacement miners found for file {cid}"
                    )
                ...
                if len(replacement_miners) < failing_count:
                    logger.warning(
                        f"Only found {len(replacement_miners)} replacement miners for {failing_count} failing miners"
                    )
                ...
                for i, miner in enumerate(new_miners):
                    if miner in failing_miners and replacement_idx < len(
                        replacement_miners
                    ):
                ...
                logger.info(
                    f"Successfully queued replacement for file {cid[:16]}... - "
                    f"replacing {len(failing_miners)} failing miners with: {', '.join(replacement_miners)}"
                )

            except Exception as e:
                logger.error(
                    f"Error processing failing miner replacement for file {file_info.get('cid', 'unknown')}: {e}"
                )
                failed_replacements += 1
                continue
    ...
    async def process_file_assignments(self) -> None:
        ...
        try:
            ...
            logger.info(
                "🎯 PRIORITY 1: Scanning file_assignments table for NULL miners..."
            )
            reassign_successful, reassign_failed = await self.process_reassignments(
                available_miners, current_epoch
            )

            # PRIORITY 2: Process new file assignments from pending table
            logger.info(
                "📋 PRIORITY 2: Processing new files from pending_assignment_file..."
            )
            new_successful, new_failed = await self.process_new_file_assignments(
                available_miners, current_epoch
            )

            # PRIORITY 3: Process failing miner replacements
            logger.info("🔄 PRIORITY 3: Processing failing miner replacements...")
            replace_successful, replace_failed = (
                await self.process_failing_miner_replacements(
                    available_miners, current_epoch
                )
            )
            ...
            logger.info(
                f"  🎯 NULL miner fixes: {reassign_successful} successful, {reassign_failed} failed"
            )
            logger.info(
                f"  📋 New assignments: {new_successful} successful, {new_failed} failed"
            )
            logger.info(
                f"  🔄 Failing replacements: {replace_successful} successful, {replace_failed} failed"
            )
            logger.info(
            ...
            async with self.db_pool.acquire() as conn:
                ...
                if remaining_nulls == 0:
                    logger.info(
                        "🎉 SUCCESS: All files now have complete miner assignments!"
                    )
                else:
                    logger.warning(
    ...
    def _log_distribution_stats(self):
        ...
        logger.info(
            f"  Assignment range: {min_assignments} - {max_assignments} per miner"
        )
    ...
    async def get_files_with_failing_miners(
