...
class FileAssignmentConsumer:
    ...
    async def process_reassignment(self, assignment_data: Dict[str, Any]) -> bool:
        ...
        if null_miner_count == 5:
            logger.info(
                f"🎯 Processing COMPLETELY EMPTY assignment for file {filename} ({cid[:16]}...) - filling all 5 slots"
            )
        elif null_miner_count > 0:
            logger.info(
                f"📋 Processing PARTIAL reassignment for file {filename} ({cid[:16]}...) - filling {null_miner_count} empty slots"
            )
        else:
            logger.info(
                f"🔄 Processing reassignment for file {filename} ({cid[:16]}...) - adding {len(new_miners)} miners"
            )
        ...
        try:
            async with self.db_pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    current_list = [
                        current_assignment["miner1"],
                        current_assignment["miner2"],
                        current_assignment["miner3"],
                        current_assignment["miner4"],
                    ...
                    if final_null_count == 0:
                        logger.info(
                            f"✅ Successfully COMPLETED assignment for file {cid[:16]}... - ALL 5 slots now filled!"
                        )
                    else:
                        logger.info(
    ...
    async def process_failing_miner_replacement(self, assignment_data: Dict[str, Any]) -> bool:
        ...
        if not cid or not owner or not new_miners:
            logger.error(
                f"Invalid failing miner replacement data: missing cid, owner, or new_miners. Data: {assignment_data}"
            )
            return False

        logger.info(
            f"Processing failing miner replacement for file {filename} ({cid[:16]}...) - "
        ...
        try:
            async with self.db_pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    logger.info(
                        f"Successfully replaced failing miners for file {cid[:16]}... - "
                    ...
                    logger.info(
                        f"File assignment updated - user profile for {owner} will be reconstructed in next cycle"
                    )
    ...
    async def process_rebalancing(self, assignment_data: Dict[str, Any]) -> bool:
        ...
        logger.info(
            f"🔄 Processing rebalancing for file {filename} ({cid[:16]}...) - "
        ...
        try:
            async with self.db_pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    logger.info(
                        f"✅ Successfully rebalanced file {cid[:16]}... - "
    ...
    async def message_handler(self, message: IncomingMessage):
        ...
        async with message.process():
            try:
                ...
                if success:
                    logger.debug(
                        f"Successfully processed {assignment_data.get('type', 'assignment')} for {assignment_data.get('cid', 'unknown')}"
                    )
                else:
                    logger.error(
                        f"Failed to process {assignment_data.get('type', 'assignment')} for {assignment_data.get('cid', 'unknown')}, discarding message"
                    )
