...
class NetworkSelfHealingProcessor:
    ...
    async def get_known_miners(self) -> List[str]:
        ...
        async with self.db_pool.acquire() as conn:
            ...
            return [row["node_id"] for row in miners]
    ...
    async def find_deregistered_miners(self) -> List[str]:
        ...
        for miner_node_id in known_miners:
    ...
    async def process_self_healing(self):
        ...
        try:
            ...
            if deregistered_miners:
                ...
                hippius_substrate = connect_to_node(node_url)
                keypair = Keypair.create_from_mnemonic(validator_seed, ss58_format=42)
                logger.info(
                    f"Submitting deregistration report to Hippius using account: {keypair.ss58_address}"
                )
