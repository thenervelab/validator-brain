...
from substrate_fetcher.ipfs_health_utils import (
    perform_ipfs_ping,
    perform_ipfs_pin_check,
)
...
class EpochHealthConsumer:
    ...
    async def process_epoch_health_check(self, message_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Processing {check_type} health check for miner {node_id} (IPFS: {ipfs_peer_id}) in epoch {epoch}"
            )
            logger.info(
                f"Miner has {total_files} files assigned, checking {min(len(files), self.max_files_per_miner)} files"
            )
            ...
            if files and ping_successful:
                ...
                logger.info(
                    f"Completed epoch pin tests for {node_id}: {successful_checks} successful, {failed_checks} failed"
                )
    ...
    async def update_epoch_summary(
        self,
        node_id: str,
        epoch: int,
        ...
        async with self.db_pool.acquire() as conn:
            try:
                ...
                logger.debug(
                    f"Updated epoch summary for {node_id}: +{successful_checks} successes, +{failed_checks} failures"
                )
