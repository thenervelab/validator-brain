...
class AvailabilityManagerProcessor:
    ...
    async def check_empty_assignments(self) -> dict:
        ...
        stats = {"files_checked": 0, "files_fixed": 0, "files_failed": 0}
        ...
        try:
            ...
            logger.info(
                f"Empty assignments check complete: {stats['files_fixed']} fixed, {stats['files_failed']} failed"
            )
            return stats
    ...
    async def process_availability_failures(self) -> dict:
        ...
        stats = {"files_checked": 0, "files_reassigned": 0, "reassignment_failures": 0}
        ...
        try:
            ...
            if stats["files_checked"] > 0:
                logger.info(
                    f"Availability reassignments complete: {stats['files_reassigned']} reassigned, {stats['reassignment_failures']} failed"
                )
            else:
