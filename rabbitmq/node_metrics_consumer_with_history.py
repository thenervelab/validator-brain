"""
RabbitMQ consumer for processing node metrics messages with history retention.
...
4. Keeps only the last N blocks of history for each miner
"""
...
# Number of historical blocks to keep per miner
HISTORY_BLOCKS = int(os.getenv('NODE_METRICS_HISTORY_BLOCKS', '10'))
...
class NodeMetricsConsumer:
    ...
    async def store_node_metrics(self, metrics: Dict[str, Any]) -> bool:
        """
        Store node metrics in the database, keeping only the last N blocks.
            ...
            async with self.db_pool.acquire() as conn:
                ...
                async with conn.transaction():
                    ...
                    await conn.execute("""
                        ...
                        ON CONFLICT (miner_id, block_number) 
                        DO UPDATE SET
                            ipfs_repo_size = EXCLUDED.ipfs_repo_size,
                            ipfs_storage_max = EXCLUDED.ipfs_storage_max,
                    ...
                    # Delete old records, keeping only the last N blocks
                    ...
                    """, metrics['miner_id'], HISTORY_BLOCKS)
                
                logger.info(f"Stored metrics for miner {metrics['miner_id']} at block {metrics['block_number']} (deleted {deleted} old records, keeping last {HISTORY_BLOCKS} blocks)")
                return True
    ...
    async def start_consuming(self):
        ...
        logger.info(f"Starting to consume from queue '{self.queue_name}' (keeping {HISTORY_BLOCKS} blocks of history)")
