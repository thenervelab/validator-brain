...
) -> Tuple[int, int, SubstrateInterface]:
    """
...
def is_epoch_validator(
    substrate: SubstrateInterface, our_validator_account: str
) -> Tuple[bool, Optional[str], Optional[int], SubstrateInterface]:
    """
    ...
    for attempt in range(max_retries):
        try:
            ...
            result = current_substrate.query(
                module="IpfsPallet", storage_function="CurrentEpochValidator"
            )
            ...
            logger.warning(
                f"Failed to check epoch validator (attempt {attempt + 1}/{max_retries}): {e}"
            )
            if attempt < max_retries - 1:
                time.sleep(5)  # Wait before retry
                # Try to reconnect
...
def connect_substrate() -> SubstrateInterface:
    ...
    for attempt in range(max_retries):
        try:
            logger.info(
                f"Connecting to substrate at {NODE_URL} (attempt {attempt + 1}/{max_retries})"
            )
