...
async def upload_profiles_to_ipfs(user_profiles, miner_profiles):
    ...
    try:
        ...
        for owner, entries in user_profiles.items():
            ...
            for entry in entries:
                ...
                if file_hash:
                    file_hash_encoded = list(
                        bytes.fromhex(file_hash.encode("utf-8").hex())
                    )
            ...
            # Upload to IPFS
            result = await upload_json_to_ipfs(
                data=encoded_entries, api_url=ipfs_node_url
            )
            if result["success"]:
                profile_cids["users"][owner] = result["cid"]
                logger.info(
                    f"Uploaded user profile for {owner} to IPFS: {result['cid']}"
                )
            else:
                logger.error(
                    f"Failed to upload user profile for {owner}: {result['error']}"
                )
        ...
        for miner_id, entries in miner_profiles.items():
            ...
            if result["success"]:
                ...
                logger.info(
                    f"Uploaded miner profile for {miner_id} to IPFS: {result['cid']}"
                )
            else:
                logger.error(
                    f"Failed to upload miner profile for {miner_id}: {result['error']}"
                )
...
async def process_completed_storage_requests(conn, user_profiles):
    ...
    try:
        ...
        processed_requests = await conn.fetch(
            "SELECT owner, file_hash, main_req_hash, selected_miners FROM pending_pool WHERE status = $1",
            "processed",
        )
        ...
        for request in processed_requests:
            ...
            if owner in user_profiles and user_profiles[owner]:
                ...
                if result["success"]:
                    ...
                    await delete_completed_storage_request(
                        conn, owner, file_hash, main_req_hash
                    )
                else:
                    logger.error(
                        f"Failed to upload user profile for {owner}: {result['error']}"
        ...
        logger.info(
            f"Prepared {len(pin_requests)} pin requests for blockchain submission"
        )
        return pin_requests
