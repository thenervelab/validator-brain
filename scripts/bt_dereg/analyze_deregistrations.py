...
def fetch_linked_nodes(substrate: SubstrateInterface) -> Dict[str, List[str]]:
    """Fetch linked nodes from Registration.LinkedNodes - FIXED for substrate aggregation issue.
    
    The substrate-interface library incorrectly aggregates LinkedNodes data into one massive entry.
    This function documents the issue and returns the best available data with clear warnings.
    
    Based on polkadot.js raw data structure:
    [[primary_node1], [linked_node1, linked_node2, ...]]
    [[primary_node2], [linked_node3]]
    ...
    
    But substrate-interface returns: one_primary -> [all_286_linked_nodes]
    
    Returns:
        Dict mapping primary_node_id -> [linked_node1, linked_node2, ...]
    """
    logger.info("=== Fetching Linked Nodes (FIXED for aggregation issue) ===")
    
    # Get the raw result directly from substrate-interface
    try:
        ...
        if len(result_list) == 0:
            logger.warning("No LinkedNodes data found")
            return {}
            
        if len(result_list) == 1:
            # This is the aggregation issue - substrate-interface returned one massive entry
            ...
            if isinstance(single_entry, (list, tuple)) and len(single_entry) == 2:
                ...
                logger.warning("=== SUBSTRATE-INTERFACE AGGREGATION ISSUE CONFIRMED ===")
                logger.warning(f"Found only 1 entry with {len(linked_array) if isinstance(linked_array, list) else 'unknown'} linked nodes")
                logger.warning(f"Primary node: {primary_key}")
                logger.warning("")
                logger.warning("ANALYSIS:")
                logger.warning("  Expected: Multiple separate [[primary], [linked...]] entries")
                logger.warning("  Actual:   One massive {primary} -> [all_linked_nodes] entry")
                logger.warning("  Cause:    substrate-interface library aggregation bug")
                logger.warning("")
                logger.warning("IMPACT:")
                logger.warning("  - All linked nodes incorrectly assigned to one primary node")
                logger.warning("  - Cannot restore original primary->linked relationships")
                logger.warning("  - coldkey -> node mappings will be incomplete")
                logger.warning("")
                logger.warning("RETURNING AGGREGATED DATA WITH CLEAR LIMITATIONS")
                
                if isinstance(linked_array, list):
                    return {str(primary_key): [str(node) for node in linked_array]}
                else:
                    return {str(primary_key): [str(linked_array)] if linked_array else []}
        
        # If we have multiple entries, process them normally (this would be the correct case)
        logger.info("Processing multiple LinkedNodes entries (correct structure)")
        linked_nodes = {}
...
def build_coldkey_node_mapping(bittensor_coldkeys: Set[str], 
                               primary_nodes: Dict[str, str],
                               linked_nodes: Dict[str, List[str]]) -> Dict[str, List[str]]:
...
def analyze_discrepancies(bittensor_coldkeys: Set[str], 
                         primary_nodes: Dict[str, str],
                         coldkey_mapping: Dict[str, List[str]]):
...
def save_results(coldkey_mapping: Dict[str, List[str]], discrepancies: Dict, output_file: str = "deregistration_analysis.json"):
