...
from ipfs_api import ping_ipfs_node, check_cid_is_provided, get_cid_blocks
...
async def update_miner_health_metrics(health_results: List[Dict]):
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            ...
            async with conn.transaction():
                for result in health_results:
                    ...
                    ping_time_ms = result['ping_time_ms'] or 0

                    # Extract content verification stats
                    content_verification = result.get('content_verification', {})
                    success_rate = content_verification.get('success_rate', 0)
                    ...
                    await conn.execute("""
                        ...
                        (node_id, is_online, ping_time_ms, pin_check_successes, pin_check_failures, 
                         success_rate, checked_at)
                        VALUES 
                        ($1, $2, $3, $4, $5, $6, NOW())
                        ON CONFLICT (node_id) 
                        DO UPDATE SET
                            is_online = $2,
                            ping_time_ms = $3,
                            pin_check_successes = $4,
                            pin_check_failures = $5,
                            success_rate = $6,
                            checked_at = NOW()
                        """, node_id, is_online, ping_time_ms, successful_cids,
                        total_cids - successful_cids, success_rate)

                    # Update miners table
                    await conn.execute("""
                        UPDATE miners
                        SET last_online = CASE WHEN $2 THEN NOW() ELSE last_online END,
                            miner_success_rate = $3,
                            updated_at = NOW()
                        WHERE node_id = $1
                        """, node_id, is_online, success_rate)
        ...
        logger.error(f"Error updating miner health metrics: {e}")
        raise
...
async def get_offline_miners():
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            rows = await conn.fetch("""
                SELECT m.node_id, r.ipfs_node_id, m.miner_profile_cid, m.last_online
                FROM miners m
                JOIN registration r ON m.node_id = r.node_id
                WHERE m.is_online = false OR m.last_online < NOW() - INTERVAL '1 hour'
                """)

            for row in rows:
                # Verify the miner is actually offline with a ping
                ipfs_node_id = row['ipfs_node_id']
                if ipfs_node_id:
                    ping_result = await ping_ipfs_node(ipfs_node_id)

                    if not ping_result['success']:
                        offline_miners.append(
                            {'node_id': row['node_id'], 'ipfs_node_id': ipfs_node_id,
                                'miner_profile_cid': row['miner_profile_cid'],
        ...
        logger.error(f"Error getting offline miners: {e}")
